...
form.setFieldsValue({...data?.company,sizes:data?.company.design_sizes})
}
...
// console.log(token,"token")
  ...
  const { isLoading: isCompanyByIDLoading} = useGetCompanyById(id,onGetByIdSuccess);
  ...
  return (
    ...
    >
      {isCompanyByIDLoading && id ? <LoadingOutlined style={{ fontSize: 24 }} spin /> :
      <Form form={form} 
      // initialvalue={{...companyDataByID?.company,sizes:companyDataByID?.company.design_sizes}} 
      layout="vertical" name="form_in_modal">
        <Row gutter={[20, 20]}>
